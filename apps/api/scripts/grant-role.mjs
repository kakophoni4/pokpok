...
if (!listOnly) {
  if (!nickname) fail('Usage: node scripts/grant-role.mjs "<nickname>" [player|moderator|admin]');
...
async function printRoster() {
  const { rows } = await client.query(
    `SELECT nickname, role, status, "createdAt"
       FROM "User"
      ORDER BY CASE role WHEN 'admin' THEN 0 WHEN 'moderator' THEN 1 ELSE 2 END, "createdAt"`,
  );
